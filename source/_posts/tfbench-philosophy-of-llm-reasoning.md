...
But looking like reasoning and *being* reasoning are two very different things.
...
Now what does a human see? Meaningful variable names, comments, naming conventions --- the natural language layer that helps us *understand* code. Call this **cognitive semantics**. It has zero effect on execution.
...
Here is a question most people never ask: if every proposed foundation for mathematics has cracked, what are mathematicians actually *doing*?
...
The mathematician does not need to commit to whether numbers "really exist." She takes her axioms *as if* they were true and reasons downward to a conclusion. Start from assumptions. Apply derivation rules. Arrive at results. **Truth comes first; existence follows.**
...
Category theory. But not as yet another metaphysical foundation --- Landry views it as a **methodological metalanguage**: a framework for talking about structured systems (sets, groups, topological spaces, deductive systems) without committing to what those systems ultimately *are*. The category axioms are taken *as if* they were first principles. They are hypotheses set up for the purpose of solving problems.

This is **methodological structural as-ifism**: what matters is the *method* --- reasoning from structure alone --- not whether structures "really exist."
...
A function of type $A \rightarrow B$ is simultaneously a proof that $A$ implies $B$ *and* a morphism from object $A$ to object $B$. The implementation *is* the proof. The type signature *is* the proposition. This is not metaphor. It is a mathematical correspondence --- and the foundation of Haskell's type system.
...
- **Type dependencies** = **axioms** --- the assumptions we take *as if* they were first principles.
...
When the AI community says a model "reasons," it is making a claim about *method*. But we evaluate that claim by looking at *outputs* --- accuracy on benchmarks.

**We are conflating method with result.** The same error Landry identifies in 2,400 years of philosophy of mathematics.

A model gets 90% on a math benchmark by pattern-matching to training data. It generates chain-of-thought that *looks* like reasoning but is post-hoc rationalization. It solves code tasks by recognizing variable names without understanding type-theoretic structure.

None of this is reasoning. Reasoning requires three things:
...
The field is making an error analogous to what Landry identifies in 2,400 years of philosophy of mathematics: **confusing the appearance of reasoning with its substance.** Chain-of-thought *looks like* human reasoning. We take that as evidence it *is* reasoning. But strip the surface, test the structure, and the performance tells a different story.
...
Landry's as-ifism gives us the test. The model should take the provided axioms *as if* they were first principles and deduce the conclusion from logical structure alone. Names are cognitive scaffolding for human readers. They are not logical content. **The model should reason about the morphism, not the label on the object.**
...
3. **Metrics that measure reasoning, not accuracy.** RS and RE are initial proposals. The field needs more tools to understand *how* models arrive at answers, not just whether they do.
