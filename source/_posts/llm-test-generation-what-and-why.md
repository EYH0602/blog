...
## How my thinking got here: a three-year roadmap

I did not start with the claim above; I backed into it, one paper at a time. Each project set out to fix a concrete problem, and each time solving it exposed the next gap — until the gaps lined up into a single argument. Here is the actual sequence of what I was thinking, and what I did about it.

**First I thought the problem was data (UniTSyn, 2024).** My starting worry was mundane: a model cannot generate a test that encodes intent if it never learned what intent *attaches* to. Models trained on undifferentiated code see tests and functions as unrelated token streams; they have no notion that this assertion is *about* that function. So the thing to fix was the training signal. I paired every test with its focal function at scale — 2.7M pairs across five languages — mined language-agnostically through the Language Server Protocol instead of brittle per-language heuristics. The lesson I took away: knowing what you are asserting on is the *prerequisite* for an oracle, not the oracle itself. Which raised the next question.

**Then I realized meaning without diversity is not enough (FuzzAug, 2025).** Correctly-paired tests taught the model what intent attaches to, but human-written tests are scarce and narrow — they exercise a handful of tidy inputs. Fuzzing inputs, meanwhile, are endlessly diverse but carry no oracle at all. So I did the deliberately ironic thing: I took fuzzing, the canonical *no-oracle* technique, and fed its coverage-guided inputs into semantically valid test templates that keep the human-written assert. Strip fuzzing of its weakness (meaningless inputs, no oracle), keep its strength (diverse behavior), fuse it onto the model's oracle. That was the first time I framed dynamic analysis as being *in service of* intent rather than a replacement for it — and it made me ask what happens when there is no reference behavior to lean on at all.

**Which forced the real problem into the open (LISA, 2026).** By now the residual gap was undeniable: even a meaningful test on diverse inputs is useless if you have no oracle to judge the output, and the bugs I actually cared about — silent functional bugs — never crash, so nothing mechanical tells you the output is wrong. This is the frontal assault. Name the enemy (silent functional bugs), name why fuzzing structurally cannot reach them (no signal), and build the intent-oracle explicitly: documentation-grounded invariants inserted at chunk boundaries, verified against a reference build, as *weaker-but-robust* oracles for programs that are otherwise non-testable. The key design move is **decoupling** exploration from oracle construction. And I kept an honesty clause on purpose — LISA emits high-confidence bug *candidates* for a developer to confirm. It mitigates the oracle problem; it does not solve it.

Read in order, the three are not three tools. They are one claim assembled backward from its symptoms: fix the data, then the diversity, then the oracle — and what falls out the far end is *manufacture intent-based oracles, at scale, grounded in program analysis.* The last gap, the one LISA only mitigates, is what the next section is about.
