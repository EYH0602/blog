...
That is when the productivity pitch fell apart for me. If AI testing is only *convenient*, it is fighting on a crowded axis — a better fuzzer, a better template engine, they compete there too, and AI might lose. The argument worth making is a **capability** argument: there is an entire class of bug that emits no mechanical signal, that every traditional method misses not by weakness but by category, and catching it requires inferring *intent* from natural language. That is exactly, and only, what LLMs are for.
