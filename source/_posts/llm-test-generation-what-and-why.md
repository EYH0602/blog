...
Over the past few months, at nearly every event and conference, a friend eventually asks me some version of the same question: now that models like [Fable 5 and Mythos 5](https://www.anthropic.com/news/claude-fable-5-mythos-5) write the code *and* the tests, what is left to do in software testing? Isn't it done? A lot of people seem to quietly wonder the same thing, and the honest answer runs against the premise of the question.
...
- *Corollary — the **definition-by-absence trap**:* automated bug detection presupposes a definition of the bug you are hunting; you cannot mechanically find what you cannot state. The field still defines functional bugs by absence ("no crash," "silent," "machine un-auditable"), so the LLM currently stands in as a stopgap *oracle*, guessing the answer where a specification should be — the guessing is the stopgap, not the model.
...
**The first obstacle is attachment: a model cannot infer a meaningful oracle unless it knows which behavior the test is asserting about.** Models trained on undifferentiated code see tests and functions as unrelated token streams — they have no notion that this assertion is *about* that function. [UniTSyn (ISSTA 2024)](https://arxiv.org/abs/2402.03396) was my attempt to fix that training signal: 2.7M tests correctly paired with their focal functions across five languages, mined language-agnostically through the Language Server Protocol.
